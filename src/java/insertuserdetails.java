...
public class insertuserdetails extends HttpServlet {
    ...
    public void service(HttpServletRequest request, HttpServletResponse response)
        ...
        e.printStackTrace();
            ...
            db.pstmt=db.con.prepareStatement("insert into userdetails values('"+value[3]+"','"+value[0]+"','"+value[1]+"','"+value[2]+"','"+fileName+"')");
            int j=db.pstmt.executeUpdate();
            
            if(j>0)
            {
              response.sendRedirect("onlineexam2.jsp?msg=Successfully Registered");
              
               
               
            }
            else{
               response.sendRedirect("onlineexam2.jsp?msg=Error");
